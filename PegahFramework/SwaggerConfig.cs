...
public static class SwaggerConfig
{
    public static void ConfigureSwaggerService(WebApplicationBuilder builder)
    {
        ...
        builder.Services.AddSwaggerGen(c =>
        {
            ...
            {
                Title = "PegahFramework API",
                Version = "v1",
                Description = @"Welcome to the PegahFramework API, which allows you to seamlessly integrate PegahFramework with your application.

### <a name=""authentication""></a> Authentication
This API is secured by an API Key.The API Key has either to be sent in the `X-ApiKey` HTTP Header:

```
GET http://localhost/PegahFramework/api/resource
...
```
GET http://localhost/PegahFramework/api/resource?apiKey=YOUR_API_KEY
```",
            });
