...
resource sqlserver 'Microsoft.Sql/servers@2021-11-01-preview' = if(usePostgressDatabase == false) {
  name: 'pegah-framework-server-${suffix}'
  location: location
  properties: {
  ...
  resource database 'databases' = {
    name: 'pegah-framework-database-${suffix}'
    location: location
    sku: {
      name: 'Basic'
      tier: 'Basic'
...
var brodcastUrls = 'https://pegah-framework-app-${suffix}.azurewebsites.net;https://pegah-framework-app-${suffix}-behind.azurewebsites.net'

resource servicePlan 'Microsoft.Web/serverfarms@2021-03-01' = {
  name: 'pegah-framework-service-plan-${suffix}'
  location: location
  kind: 'linux'
  properties: {
    reserved: true
...
resource app 'Microsoft.Web/sites@2021-03-01' = {
  name: 'pegah-framework-app-${suffix}'
  location: location
